...
describe('[Challenge] Climber', function () {
  ...
  it('Execution', async function () {
    ...
    let salt = ethers.utils.id('aleko');

    const Attacker = await ethers.getContractFactory('AttackerClimber');
    ...
    const FakeClimberVault = await ethers.getContractFactory(
      'FakeClimberVault'
    );
    const fakeClimberVault = await FakeClimberVault.deploy();

    await attacker.attack(
      fakeClimberVault.address,
      vault.address,
      token.address
    );
  });
